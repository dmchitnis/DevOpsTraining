...
stage('Run tests'){
    node {
        ...
        dir('UI'){
            def sqScannerMsBuildHome = tool 'sonarqube-default';
            withSonarQubeEnv('sonarqube-1','DevOpsTraining') { // If you have configured more than one global server connection, you can specify its name
